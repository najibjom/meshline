...
describe("Meshline 1.0.8 update action contract", () => {
  it("keeps a direct, visible update action outside the status card", () => {
    ...
    expect(screenSource).toContain("style={[styles.updateAction, working && styles.updateActionDisabled]}");
  ...
  it("defines check, download, and restart states for the one visible action", () => {
    ...
    expect(screenSource).toContain('const MESHLINE_BASE_RUNTIME = "1.0.8";');
  });
});
