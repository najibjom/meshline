    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.select({ ios: "padding", default: undefined })} keyboardVerticalOffset={0}>
        ...
        <View style={[styles.composerWrap, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
          ...
          {canPost && !conversation.isGuide ? <View style={[styles.composer, { backgroundColor: colors.background }]}><TextInput value={draft} onChangeText={setDraft} placeholder={isChannel ? "Publish an update" : conversation.isSavedMessages ? "Write a note" : "Write a message"} placeholderTextColor={colors.muted} style={[styles.composerInput, { color: colors.text }]} multiline maxLength={2000} returnKeyType="default" /><Pressable onPress={send} disabled={!canSend} accessibilityLabel="Send message" accessibilityState={{ disabled: !canSend }} style={({ pressed }) => [styles.send, !canSend && styles.sendDisabled, pressed && canSend && styles.pressed]}><MaterialIcons name="arrow-upward" size={20} color={canSend ? "#FFFFFF" : palette.indigo} /></Pressable></View> : <View style={[styles.readOnlyComposer, { backgroundColor: colors.background }]}><MaterialIcons name={conversation.isGuide ? "info-outline" : !isCurrentSpaceMember ? "person-remove" : isChannel ? "campaign" : "lock-outline"} size={18} color={colors.muted} /><Text style={[styles.readOnlyText, { color: colors.muted }]}>{conversation.isGuide ? "Meshline Guide is read-only" : !isCurrentSpaceMember ? "You are no longer a member of this space" : isChannel ? "Only the channel owner can post" : "Only group owners can post"}</Text></View>}
...
const styles = StyleSheet.create({
  ...
  composer: { minHeight: 50, maxHeight: 120, borderRadius: 18, backgroundColor: "#F0F2F7", flexDirection: "row", alignItems: "flex-end", paddingLeft: 15, paddingRight: 5, paddingVertical: 5 },
  readOnlyComposer: { minHeight: 50, borderRadius: 18, backgroundColor: "#F0F2F7", flexDirection: "row", gap: 8, alignItems: "center", justifyContent: "center", paddingHorizontal: 14 },
  ...
  send: { width: 40, height: 40, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: palette.indigo, marginLeft: 5, shadowColor: palette.navy, shadowOpacity: 0.15, shadowRadius: 7, shadowOffset: { width: 0, height: 3 }, elevation: 2 },
