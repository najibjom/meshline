...
export default function GroupSettingsScreen() {
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-[#F6F7FB]" className="bg-[#F6F7FB]">
      <View style={styles.header}><Pressable onPress={() => router.back()} hitSlop={10} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcons name="arrow-back" size={22} color={palette.ink} /></Pressable><Text style={styles.headerTitle}>Edit group</Text><View style={styles.headerSpacer} /></View>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator>
        <SectionCard style={styles.identityCard}><View style={styles.groupMark}><MaterialIcons name="group" size={26} color={palette.indigo} /></View><View style={styles.identityCopy}><Text style={styles.groupName}>{group.peerDisplayName}</Text><Text style={styles.groupHandle}>{group.peerUsername}</Text><Text style={styles.groupMeta}>Local group · {memberCount} member{memberCount === 1 ? "" : "s"}</Text></View></SectionCard>
        {isOwner ? <><Text style={styles.sectionTitle}>GROUP INFO</Text><SectionCard style={styles.formCard}><Text style={styles.label}>Group name</Text><TextInput value={title} onChangeText={(value) => { setTitle(value); setError(""); }} maxLength={60} placeholder="Group name" placeholderTextColor="#9CA5B5" style={styles.input} returnKeyType="next" /><Text style={styles.label}>Public @username</Text><TextInput value={username} onChangeText={(value) => { setUsername(value.toLowerCase().replace(/[^a-z0-9_@]/g, "")); setError(""); }} maxLength={25} autoCapitalize="none" autoCorrect={false} placeholder="@group" placeholderTextColor="#9CA5B5" style={styles.input} returnKeyType="next" /><Text style={styles.label}>Description</Text><TextInput value={description} onChangeText={(value) => { setDescription(value); setError(""); }} maxLength={180} multiline placeholder="Describe this group" placeholderTextColor="#9CA5B5" style={[styles.input, styles.descriptionInput]} />{error ? <Text style={styles.error}>{error}</Text> : null}<Pressable onPress={() => void save()} style={({ pressed }) => [styles.saveButton, pressed && styles.pressed]}><Text style={styles.saveText}>Save group changes</Text></Pressable></SectionCard><Text style={styles.note}>These settings are stored on this device in the current local-first MVP. Shared group identity will be handled by a future encrypted network protocol.</Text></> : <SectionCard style={styles.readOnlyCard}><MaterialIcons name="lock-outline" size={21} color={palette.muted} /><View style={styles.readOnlyCopy}><Text style={styles.readOnlyTitle}>Only the group owner can edit settings</Text><Text style={styles.readOnlyText}>The creator controls this group’s name, @username, and description.</Text></View></SectionCard>}</ScrollView>
    </ScreenContainer>
  );
}
