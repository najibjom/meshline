...
function labelFor(state: OtaUpdateState) {
...
export default function AppUpdatesScreen() {
  ...
  const check = async () => {
    ...
    try {
      ...
      if (currentNetwork.isInternetReachable !== true) { setState("error"); setMessage("No internet connection on this device. Connect to Wi-Fi or mobile data, then try again."); haptic.warning(); return; }
  ...
  const download = async () => {
    setWorking(true); setMessage("Downloading the compatible Meshline update…");
    try { const result = await downloadMeshlineUpdate(); setState(result.state); setMessage(result.message); result.state === "error" ? haptic.warning() : haptic.light(); }
    finally { setWorking(false); }
  };
  const apply = async () => {
    setWorking(true); setMessage("Restarting Meshline to apply the downloaded update…");
    try { await applyDownloadedMeshlineUpdate(); }
    catch { setWorking(false); setState("error"); setMessage("Meshline could not restart to apply the update. Please reopen the app and try again."); haptic.warning(); }
  };
