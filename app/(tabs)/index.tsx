...
function ConversationRow({ conversation, latest, onPress }: { conversation: { peerDisplayName: string; peerUsername: string; updatedAt: string; unreadCount?: number; kind?: "direct" | "group" | "channel"; isGuide?: boolean; isSavedMessages?: boolean; isPinned?: boolean }; latest?: Message; onPress: () => void }) {
  const colors = useColors();
    ...
    <Pressable onPress={onPress} style={({ pressed }) => [styles.conversation, { borderBottomColor: colors.border }, pressed && styles.pressed]}>
      ...
      <View style={styles.conversationCopy}>
        <View style={styles.conversationTop}>
          ...
          <Text style={[styles.conversationTime, { color: colors.muted }]}>{formatConversationTime(conversation.updatedAt)}</Text>{conversation.unreadCount ? <View style={styles.unreadBadge}><Text style={styles.unreadBadgeText}>{conversation.unreadCount > 99 ? "99+" : conversation.unreadCount}</Text></View> : null}
        </View>
        <View style={styles.previewRow}>
          {conversation.isSavedMessages ? <Text style={styles.spaceLabel}>PERSONAL</Text> : conversation.kind && conversation.kind !== "direct" ? <Text style={styles.spaceLabel}>{conversation.kind === "group" ? "GROUP" : "CHANNEL"}</Text> : null}<Text numberOfLines={1} style={[styles.preview, { color: conversation.unreadCount ? colors.text : colors.muted }, conversation.unreadCount ? styles.unreadPreview : null]}>{preview}</Text>
...
const styles = StyleSheet.create({
  ...
  compose: { width: 52, height: 52, borderRadius: 17, backgroundColor: palette.indigo, justifyContent: "center", alignItems: "center", shadowColor: palette.navyDeep, shadowOpacity: 0.28, shadowRadius: 11, shadowOffset: { width: 0, height: 5 }, elevation: 4 },
  searchWrap: { height: 56, backgroundColor: palette.surface, borderRadius: 18, flexDirection: "row", alignItems: "center", paddingHorizontal: 16, gap: 10 },
  search: { flex: 1, height: "100%", color: palette.ink, fontSize: 17 },
